    ...
    public function userHasAuthentication(MvcEvent $e)
    {
        ...
        if ($routeMatch) {
            ...
            if ($routeName !== 'sign') {
                ...
                if (!$session->isAdmin) {
                    if ($routeName === 'settingsForAdmin' || $routeName === 'posts') {
                        $routeMatch->setParam('controller', IndexController::class);
                        $routeMatch->setParam('action', 'viewSettings');
                    }
                ...
                if ($routeName === 'settingsForAdmin' || $routeName === 'settings') {
                    $routeMatch->setParam('controller', IndexController::class);
                    $routeMatch->setParam('action', 'viewSettingsForAdmin');
                }
            }
        }
    }
